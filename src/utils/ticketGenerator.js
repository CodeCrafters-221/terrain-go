...
export const generateTicket = (reservation = {}) => {
    try {
        ...
        const id = String(reservation.id || 'N/A').slice(0, 8).toUpperCase();
        const clientName = String(reservation.clientName || 'Client').substring(0, 25);
        const clientPhone = String(reservation.clientPhone || 'Non renseigné');
        const fieldName = String(reservation.fieldName || 'Terrain').substring(0, 25);
        ...
        doc.text('TERRAIN-G', 40, 10, { align: 'center' });
        doc.setFontSize(10);
        ...
        doc.text('Merci d\'avoir utilisé Terrain-G !', 40, 130, { align: 'center' });
        doc.text('Présentez ce ticket à l\'entrée du terrain.', 40, 135, { align: 'center' });

        // Date of issue
        doc.setFontSize(7);
        doc.setFont('helvetica', 'normal');
        doc.text(`Généré le ${new Date().toLocaleString()}`, 40, 145, { align: 'center' });
